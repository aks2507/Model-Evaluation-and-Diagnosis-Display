...
export default function ScrollableTabsButtonAuto(props) {
  ...
  for (let i = 0; i < numCols; i++) {
    if (cols[i] === evalList[0].data.dataset.metadata.label) continue;
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let j = 0; j < numCols; j++) {
      if (cols[j] === evalList[i].data.dataset.metadata.label) continue;
      means.push(
        evalList[i].data.dataset.metadata.description[cols[j]].mean.toFixed(2)
      );
      stds.push(
        evalList[i].data.dataset.metadata.description[cols[j]].std.toFixed(2)
      );
      mins.push(
        evalList[i].data.dataset.metadata.description[cols[j]].min.toFixed(2)
      );
      q25s.push(
        evalList[i].data.dataset.metadata.description[cols[j]]['25%'].toFixed(2)
      );
      q50s.push(
        evalList[i].data.dataset.metadata.description[cols[j]]['50%'].toFixed(2)
      );
      q75s.push(
        evalList[i].data.dataset.metadata.description[cols[j]]['75%'].toFixed(2)
      );
      maxs.push(
        evalList[i].data.dataset.metadata.description[cols[j]].max.toFixed(2)
      );
      iqrs.push(evalList[i].data.dataset.metadata.iqr[j].toFixed(2));
      mvalss.push(
        evalList[i].data.dataset.metadata.missing_values[j].toFixed(2)
      );
    }
