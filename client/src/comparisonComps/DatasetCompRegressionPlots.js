...
export default function DatasetCompRegressionPlots(props) {
  ...
  for (let i = 0; i < evalList.length; i++) {
    ...
    residuals.push(
      evalList[i].data.metadata.observed.map(
        (item, index) => item - evalList[i].data.metadata.predicted[index]
      )
    );

    const data_name =
      c === 0 ? evalList[i].data.dataset.name : evalList[i].data.model.name;
  ...
  for (let i = 0; i < evalList.length; i++) {
    if (i === 0) {
      ...
      observedVsPredictedSubplots[Number(Math.floor(i / 3))].push(
        'x' + (i + 1) + 'y' + (i + 1)
      );
      residualVsObservedSubplots[Number(Math.floor(i / 3))].push(
        'x' + (i + 1) + 'y' + (i + 1)
      );
      residualVsPredictedSubplots[Number(Math.floor(i / 3))].push(
        'x' + (i + 1) + 'y' + (i + 1)
      );
    }
  }
  ...
  let observedVsPredictedLayout = {
    ...
    grid: {
      rows: numBins,
      columns: 3,
      subplots: observedVsPredictedSubplots,
      pattern: 'independent',
  ...
  let residualVsObservedLayout = {
    ...
    grid: {
      rows: numBins,
      columns: 3,
      subplots: residualVsObservedSubplots,
      pattern: 'independent',
      roworder: 'top to bottom',
    },
    legend: { orientation: 'h' },
  ...
  let residualVsPredictedLayout = {
    ...
    grid: {
      rows: numBins,
      columns: 3,
      subplots: residualVsPredictedSubplots,
      pattern: 'independent',
      roworder: 'top to bottom',
    },
    legend: { orientation: 'h' },
