...
export default function Metrics(props) {
  ...
  if (evalList[0].data.model_type === 'regression') {
    for (let i = 0; i < numTabs; i++) {
      ...
      let rmsle =
        evalList[i].data.metadata.root_mean_squared_log_error.toFixed(2);
      let r2 =
        evalList[i].data.metadata.Coefficient_of_Determination.toFixed(2);
      ...
      rows.push(
        createDataRegression(
          evalList[i].data.name,
          mae,
          mse,
          rmse,
          rmsle,
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Paper elevation={5}>
          <Details c={1} evaluations={evalList} />
        </Paper>
      </Grid>

      <Grid item xs={12}>
        <div className={classes.plot}>
          <CompareTable rows={rows} model_type={evalList[0].data.model_type} />
        </div>
      </Grid>
